...
async function moveThumbnail(slug: string) {
  ...
  const currentMovie = images.value!.filter(
    (image: IImage) => image.id === slug
  );
  const index = images.value!.indexOf(currentMovie![0]) as number;
  const imgToMove = ref<HTMLElement | undefined>(
    thumbnails.value?.children[index] as HTMLElement | undefined
  );
  ...
  x.value =
    x.value +
    (width.value / 2 -
      (imgToMove.value!.getBoundingClientRect().left + imageWidth / 2));
}
...
<template>
  <div
    class="bg-black/10 backdrop-blur-md top-0 h-[68px] absolute overflow-hidden w-screen"
  >
    <ul
